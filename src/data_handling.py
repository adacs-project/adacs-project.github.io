...
class DataCollatorForNormSeq2Seq:
    ...
    def bias_phrases_extractor(self, features, max_bias_per_sample=15):
        ...
        phrase_candidate_revised_additional = []
        for item in phrase_candidate_revised:
            item_fragment = self.tokenizer.sp_model.DecodePieces(item.split()).split(" ")
            if len(item_fragment) > 1:
                for x in item_fragment:
                    phrase_candidate_revised_additional += [tokenizer.decode(self.tokenizer.encode(x), skip_special_tokens=True)]
        
        phrase_candidate_revised += phrase_candidate_revised_additional
        ...
        for i in range(len(features)):
            ...
            for w_idx, w in enumerate(sample_output_words[i]):
                ...
                try:
                    sample_spoken_bias_labels.extend([phrase_candidate_revised.index(w) + 1] * features[i]['inputs_length'][w_idx])
                except:
                    if random.random() < 0.5:
                        sample_spoken_bias_labels.extend([0] * features[i]['inputs_length'][w_idx])
                    else:
                        sample_spoken_bias_labels.extend([self.label_pad_token_id] * features[i]['inputs_length'][w_idx])
            spoken_bias_labels.append(sample_spoken_bias_labels)
        ...
        return phrase_candidate_ids, phrase_candidate_mask, bias_labels, spoken_bias_labels
        # pass
...
#         print("Make feature {}s".format(time.time() - start_time))
        ...
        phrase_candidate_ids, phrase_candidate_mask, samples_bias_labels, samples_spoken_bias_labels = self.bias_phrases_extractor(features)
...
def init_data(train_corpus_path='./data-bin/paper/train_general.txt',
              test_corpus_path='./data-bin/paper/val_general.txt'):
    dataset_oov = datasets.load_dataset('text', data_files={"train": train_corpus_path,
                                                            "test": test_corpus_path})
...
def preprocess_function(batch):
    ...
    for src_words, tgt_words in zip(batch["src"], batch["tgt"]):
        #augment to merge codemix phrase
...
#     test = {"src": [["cầu", "thủ", "rô nan đô"]], "tgt": [["cầu", "thủ", "ronaldo"]]}
    model, model_tokenizer = model_handling.init_model()
    data_collator = DataCollatorForNormSeq2Seq(model_tokenizer, model=model)
    import time
    start = time.time()
    batch = data_collator([split_datasets["train"][i] for i in [random.randint(0, 9000) for _ in range(0, 64)]])
    # print(batch)
