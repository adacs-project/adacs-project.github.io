...
model = EncoderDecoderSpokenNorm.from_pretrained("").eval()
...
#     return list_pre_norm
    ...
    return reformat_normed_term(text_input_list_pre_norm, spoken_norm_output, spoken_norm_score, threshold=15, debug=False), list_pre_norm
...
#         print('{}\n{}\n\n'.format(text_input, list_bias_input))
        ...
        return norm_result[0], norm_result[1]
    
import regtag
import random

x = random.choices(regtag.get_general_en_word(), k=750)
x.extend(["netware tiktoker", "yingsen", "dmitry peskov"])
random.shuffle(x)
output = format_text("mọi phương diện chuyến thăm đã được chuẩn bị kỹ lưỡng người phát ngôn điện cờ rem lin đờ mi tri pê xờ cốp", "\n".join(x))
output